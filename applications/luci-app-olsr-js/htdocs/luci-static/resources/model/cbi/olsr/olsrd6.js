...
return view.extend({
	render: function () {
		...
		s = m.section(form.TypedSection, "olsrd6", translate("General settings"));
		s.anonymous = true;
	 ...
	 poll = s.taboption("advanced", form.Value, "Pollrate", translate("Pollrate"), translate("Polling rate for OLSR sockets in seconds. Default is 0.05."));
		...
		mainip = s.taboption("general", form.Value, "MainIp", translate("Main IP"), translate("Sets the main IP (originator ip) of the router. This IP will NEVER change during the uptime of olsrd. " + "Default is ::, which triggers usage of the IP of the first interface."));
		...
		sgwuplink = s.taboption("smartgw", form.ListValue, "SmartGatewayUplink", translate("Announce uplink"), translate("Which kind of uplink is exported to the other mesh nodes. " + 'An uplink is detected by looking for a local HNA6 ::ffff:0:0/96 or 2000::/3. Default setting is "both".'));
