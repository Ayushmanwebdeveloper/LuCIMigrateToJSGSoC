...
return view.extend({
  render: function () {
    ...
    s.tab(
      ...
      !has_ipip &&
        translate(
          "Warning: kmod-ipip is not installed. Without kmod-ipip SmartGateway will not work, please install it."
        )
    );
    ...
    nicc = s.taboption(
      ...
      form.Value,
      "NicChgsPollInt",
      translate("Nic changes poll interval"),
      translate(
        'Interval to poll network interfaces for configuration changes (in seconds). Default is "2.5".'
      )
    );
    nicc.optional = true;
    ...
    lql = s.taboption(
      ...
      translate("LQ level"),
      translate(
        "Link quality level switch between hopcount and cost-based (mostly ETX) routing.<br />" +
    ...
    lqage = s.taboption(
      ...
      translate("LQ aging"),
      translate(
        "Link quality aging factor (only for lq level 2). Tuning parameter for etx_float and etx_fpm, smaller values " +
    ...
    hyst = s.taboption(
      ...
      form.Flag,
      "UseHysteresis",
      translate("Use hysteresis"),
      translate(
        "Hysteresis for link sensing (only for hopcount metric). Hysteresis adds more robustness to the link sensing " +
    ...
    mainip = s.taboption(
      ...
      translate("Main IP"),
      translate(
        "Sets the main IP (originator ip) of the router. This IP will NEVER change during the uptime of olsrd. " +
    ...
    sgw = s.taboption(
      ...
      translate("Enable"),
      translate(
        "Enable SmartGateway. If it is disabled, then " +
    ...
    sgwnat = s.taboption(
      ...
      translate("Allow gateways with NAT"),
      translate("Allow the selection of an outgoing IPv4 gateway with NAT")
    );
    ...
    sgwuplink = s.taboption(
      "smartgw",
      form.ListValue,
      "SmartGatewayUplink",
      translate("Announce uplink"),
      translate(
        "Which kind of uplink is exported to the other mesh nodes. " +
    ...
    sgwulnat = s.taboption(
      "smartgw",
      form.Flag,
      "SmartGatewayUplinkNAT",
      translate("Uplink uses NAT"),
      translate(
        "If this Node uses NAT for connections to the internet. " +
          'Default is "yes".'
      )
    );
    ...
    sgwspeed = s.taboption(
      "smartgw",
      form.Value,
      "SmartGatewaySpeed",
      translate("Speed of the uplink"),
      translate(
    ...
    sgwprefix = s.taboption(
      "smartgw",
      form.Value,
      "SmartGatewayPrefix",
      translate("IPv6-Prefix of the uplink"),
      translate(
        "This can be used " +
    ...
    willingness = s.taboption(
      "advanced",
      form.ListValue,
      "Willingness",
      translate("Willingness"),
      translate(
        'The fixed willingness to use. If not set willingness will be calculated dynamically based on battery/power status. Default is "3".'
      )
    );
    for (let i = 0; i < 8; i++) {
      willingness.value(i);
    ...
    mode = i.taboption(
      ...
      translate("Mode"),
      translate(
        "Interface mode is used to prevent unnecessary packet forwarding on switched ethernet interfaces. " +
    ...
    lqmult = i.taboption(
      ...
      form.DynamicList,
      "LinkQualityMult",
      translate("LinkQuality Multiplicator"),
      translate(
      ...
      for (var i = 0; i < value.length; i++) {
        ...
        if (v !== "") {
          ...
          if (!host || !mult) {
            return [
              null,
              "LQMult requires two values (IP address or 'default' and multiplicator) separated by space.",
            ];
          }
          if (
            !/^(\d{1,3}\.){3}\d{1,3}$|^([a-fA-F0-9]{1,4}:){7}[a-fA-F0-9]{1,4}$/.test(
              host
            ) &&
            host !== "default"
          ) {
            return [
              null,
              "Can only be a valid IPv4 or IPv6 address or 'default'",
            ];
          }
          if (isNaN(mult) || mult > 1 || mult < 0.01) {
            return [
              null,
              "Invalid Value for LQMult-Value. Must be between 0.01 and 1.0.",
            ];
          }
          if (!/^[0-1]\.\d+$/.test(mult)) {
            return [
              null,
              "Invalid Value for LQMult-Value. You must use a decimal number between 0.01 and 1.0 here.",
            ];
          }
        }
      }
    ...
    ip4b = i.taboption(
      ...
      translate(
        "IPv4 broadcast address for outgoing OLSR packets. One useful example would be 255.255.255.255. " +
    ...
    ip6m = i.taboption(
      ...
      form.Value,
      "IPv6Multicast",
      translate("IPv6 multicast"),
      translate(
        'IPv6 multicast address. Default is "FF02::6D", the manet-router linklocal multicast.'
      )
    );
    ip6m.optional = true;
    ...
    ip4s = i.taboption(
      ...
      translate(
        'IPv4 src address for outgoing OLSR packages. Default is "0.0.0.0", which triggers usage of the interface IP.'
      )
    );
    ip4s.optional = true;
    ...
    ip6s = i.taboption(
      ...
      form.Value,
      "IPv6Src",
      translate("IPv6 source"),
      translate(
        "IPv6 src prefix. OLSRd will choose one of the interface IPs which matches the prefix of this parameter. " +
