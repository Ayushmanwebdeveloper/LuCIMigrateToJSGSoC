...
return view.extend({
	render: function () {
		if(arg[1]){
		 mp = new form.Map("olsrd", translate("OLSR - Plugins"))

		p = mp.section(form.TypedSection, "LoadPlugin", translate("Plugin configuration"))
		p.depends("library", arg[1])
		p.anonymous = true
	
		ign = p.option(form.Flag, "ignore", translate("Enable"))
		ign.enabled  = "0"
		ign.disabled = "1"
		ign.rmempty  = false
		...
		lib = p.option(form.DummyValue, "library", translate("Library"))
		lib.default = arg[1]
...
const knownPlParams = {
	...
	olsrd_secure: [
			[form.Value, "Keyfile", "/etc/private-olsr.key"]
	],
	olsrd_txtinfo: [
...
return mp;
	...
	mpi = new form.Map("olsrd", translate("OLSR - Plugins"))
