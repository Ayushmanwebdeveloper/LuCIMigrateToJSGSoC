...
return view.extend({
	...
	render: function () {
	 ...
	 poll = s.taboption("advanced", form.Value, "Pollrate", _("Pollrate"), _("Polling rate for OLSR sockets in seconds. Default is 0.05."));
		...
		mainip = s.taboption("general", form.Value, "MainIp", _("Main IP"), _("Sets the main IP (originator ip) of the router. This IP will NEVER change during the uptime of olsrd. " + "Default is ::, which triggers usage of the IP of the first interface."));
		...
		sgwuplink = s.taboption("smartgw", form.ListValue, "SmartGatewayUplink", _("Announce uplink"), _("Which kind of uplink is exported to the other mesh nodes. " + 'An uplink is detected by looking for a local HNA6 ::ffff:0:0/96 or 2000::/3. Default setting is "both".'));
