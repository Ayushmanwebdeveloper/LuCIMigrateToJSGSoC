...
return view.extend({
	...
	render: function () {
		...
		m.on_parse = function () {
			...
			LuCI.uci.sections("olsrd6", "InterfaceDefaults", function (s) {
				hasDefaults = true;
				return false;
			});

			if (!hasDefaults) {
				LuCI.uci.add("olsrd6", "InterfaceDefaults");
			}
		};
		...
		function write_float(section, value) {
			...
			if (!isNaN(n)) {
				uci.set("olsrd6", section, "name", n.toFixed(1));
			}
		}
		...
		var 		fib = s.taboption("general", form.ListValue, "FIBMetric", _("FIB metric"), _("FIBMetric controls the metric value of the host-routes OLSRd sets. " + '"flat" means that the metric value is always 2. This is the preferred value ' + "because it helps the Linux kernel routing to clean up older routes. " + '"correct" uses the hopcount as the metric value. ' + '"approx" uses the hopcount as the metric value too, but does only update the hopcount if the nexthop changes too. ' + 'Default is "flat".'));
		fib.value("flat");
		fib.value("correct");
		fib.value("approx");

		var 		lql = s.taboption("lquality", form.ListValue, "LinkQualityLevel", _("LQ level"), _("Link quality level switch between hopcount and cost-based (mostly ETX) routing.<br />" + "<b>0</b> = do not use link quality<br />" + "<b>2</b> = use link quality for MPR selection and routing<br />" + 'Default is "2"'));
		lql.value("2");
		lql.value("0");

		var 		lqage = s.taboption("lquality", form.Value, "LinkQualityAging", _("LQ aging"), _("Link quality aging factor (only for lq level 2). Tuning parameter for etx_float and etx_fpm, smaller values " + "mean slower changes of ETX value. (allowed values are between 0.01 and 1.0)"));
		lqage.optional = true;
		...
		var 		hyst = s.taboption("lquality", form.Flag, "UseHysteresis", _("Use hysteresis"), _("Hysteresis for link sensing (only for hopcount metric). Hysteresis adds more robustness to the link sensing " + 'but delays neighbor registration. Defaults is "yes"'));
		hyst.default = "yes";
		hyst.enabled = "yes";
		hyst.disabled = "no";
		...
		var 	mainip = s.taboption("general", form.Value, "MainIp", _("Main IP"), _("Sets the main IP (originator ip) of the router. This IP will NEVER change during the uptime of olsrd. " + "Default is ::, which triggers usage of the IP of the first interface."));
		...
		var 	sgwuplink = s.taboption("smartgw", form.ListValue, "SmartGatewayUplink", _("Announce uplink"), _("Which kind of uplink is exported to the other mesh nodes. " + 'An uplink is detected by looking for a local HNA6 ::ffff:0:0/96 or 2000::/3. Default setting is "both".'));
		...
		var 	ip6s = i.taboption("addrs", form.Value, "IPv6Src", _("IPv6 source"), _("IPv6 src prefix. OLSRd will choose one of the interface IPs which matches the prefix of this parameter. " + 'Default is "0::/0", which triggers the usage of a not-linklocal interface IP.'));
		ip6s.optional = true;
		...
		mode.cfgvalue = function (section_id) {
			return uci.get("olsrd6", section_id, "Mode");
		};

		var 	hello = ifs.option(form.DummyValue, "_hello", _("Hello"));
		hello.cfgvalue = function (section_id) {
			var i = uci.get("olsrd6", section_id, "HelloInterval");
			var v = uci.get("olsrd6", section_id, "HelloValidityTime");
			return `${i.toFixed(1)}s / ${v.toFixed(1)}s`;
		};

		var 	tc = ifs.option(form.DummyValue, "_tc", _("TC"));
		tc.cfgvalue = function (section_id) {
			var i = uci.get("olsrd6", section_id, "TcInterval");
			var v = uci.get("olsrd6", section_id, "TcValidityTime");
			return `${i.toFixed(1)}s / ${v.toFixed(1)}s`;
		};

		var 	mid = ifs.option(form.DummyValue, "_mid", _("MID"));
		mid.cfgvalue = function (section_id) {
			var i = uci.get("olsrd6", section_id, "MidInterval");
			var v = uci.get("olsrd6", section_id, "MidValidityTime");
			return `${i.toFixed(1)}s / ${v.toFixed(1)}s`;
		};

		var 	hna = ifs.option(form.DummyValue, "_hna", _("HNA"));
		hna.cfgvalue = function (section_id) {
			var i = uci.get("olsrd6", section_id, "HnaInterval");
			var v = uci.get("olsrd6", section_id, "HnaValidityTime");
			return `${i.toFixed(1)}s / ${v.toFixed(1)}s`;
		};
